...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async event => {
        ...
        try {
            ...
            if (match && typeof saveJournalEntry === "function") {
                saveJournalEntry({ mood, location, energy, activityName: match.name });
            }

            /* Recently viewed: remember just the last recommended activity. */
            if (match) {
                localStorage.setItem("moodmateRecentActivity", JSON.stringify(match));
            }
    ...
    function showResult(activity) {
        ...
        resultArea.innerHTML = `
            <div class="result-card">
                ...
                <p class="result-saved-note">✓ Saved to your <a href="journal.html">journal</a></p>
            </div>`;
    }
});
